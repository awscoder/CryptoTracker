    ...
    with con:     
        for i in range(len(fills_CBP)):
            ...
            if fills_CBP[i][3] == "BUY":
                amt_held = (get_cur_holdings(name_database, fills_CBP[i][6])) #Current holdings for the given coin
                cur.execute(update_holdings_from_CBP_fills, ((amt_held + float(fills_CBP[i][5])), fills_CBP[i][6])) #updates holdings
                cur.execute(insert_CBP_fills, (fills_CBP[i][4], fills_CBP[i][6], float(fills_CBP[i][5]), float(fills_CBP[i][7]), fills_CBP[i][10], float(fills_CBP[i][9]), "CBP")) #adds the tranasction to the Transaction table
...
def main():
    ...
    update_transactions_and_holdings_with_CBP_fills(user_database_name) #Let's the user import the CBP fills.csv file
